...
World::World(const std::string name, const Resource *geometry, const Resource *material, const Resource *texture) : SceneNode(name, geometry, material, texture) {
	SetPosition(glm::vec3((getBounds()[0] + getBounds()[1]) / 2, getFloor(), (getBounds()[4] + getBounds()[5]) / 2));
	SetScale(glm::vec3((getBounds()[1] - getBounds()[0]) * 3, 1, (getBounds()[5] - getBounds()[4]) * 3));
}
